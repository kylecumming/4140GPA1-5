...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        let POdetails = `http://localhost:4001/api/company/x/getPODetail371/${this.state.poNo}`;
        let POLines = `http://localhost:4001/api/company/x/getPOLines371/${this.state.poNo}`;
    ...
    startTransaction371() {
        ...
        Axios.get(`http://localhost:4001/api/company/x/startTransaction371/${this.state.poNo}`).then((response) => {
    ...
    commitTransaction() {
        Axios.get('http://localhost:4001/api/company/x/endTransaction371/commit').then((res) => {
            this.setState({
    ...
    rollbackTransaction() {
        Axios.get('http://localhost:4001/api/company/x/endTransaction371/rollback').then((res) => {
            this.setState({
                transaction: 'rollback_success',
            })
        }).catch((err) => {
