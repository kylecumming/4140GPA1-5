...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        Axios.get(`http://localhost:3001/api/client/x/getPOs17/${this.state.poNo}`).then((response) => {
            this.setState({ po: response.data });
        }).catch((err) => {
            alert(err);
        });

        Axios.get(`http://localhost:3001/api/client/x/getPOLines17/${this.state.poNo}`).then((response) => {
