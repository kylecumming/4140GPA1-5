...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        let POdetails = `http://localhost:4000/api/company/w/getPODetail371/${this.state.poNo}`;
        let POLines = `http://localhost:4000/api/company/w/getPOLines371/${this.state.poNo}`;
    ...
    startTransaction371() {
        ...
        Axios.get(`http://localhost:4000/api/company/w/startTransaction371/${this.state.poNo}`).then((response) => {
    ...
    commitTransaction() {
        Axios.get('http://localhost:4000/api/company/w/endTransaction371/commit').then((res) => {
            this.setState({
    ...
    rollbackTransaction() {
        Axios.get('http://localhost:4000/api/company/w/endTransaction371/rollback').then((res) => {
            this.setState({
                transaction: 'rollback_success',
            })
        }).catch((err) => {
