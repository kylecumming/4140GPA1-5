...
export default class HomePOsList extends Component {
    ...
    constructor() {
        ...
        this.state = {
            orders: [],
            clientId: getCookie("userid") // needs a value from login
        };
        this.getPOsList17 = this.getPOsList17.bind(this);
    }

    async getPOsList17() {
        Axios.get(`http://localhost:3000/api/client/w/getPOList17/${this.state.clientId}`).then((response) => {
        ...
        return ( <
            ...
            tbody > {
                this.state.orders.map((element, index) => ( <
                    tr key = { index }
                    onClick = { this.getPODetails17.bind(this, element) } >
                    <
