...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        Axios.get(`http://localhost:3000/api/client/w/getPOs17/${this.state.poNo}`).then((response) => {
            this.setState({ po: response.data });
        }).catch((err) => {
            alert(err);
        });

        Axios.get(`http://localhost:3000/api/client/w/getPOLines17/${this.state.poNo}`).then((response) => {
    ...
    render() {
        return ( <
            ...
            h3 > Part Order number { this.state.poNo }
            details: < /h3> <
            div > Status: { this.state.po.status17 } < /div> <
            div > Date: { this.state.po.datePO17 } < /div> <
            div > Total Cost: {
                this.state.poLines.reduce(
                    (a, b) => a + b.linePrice17, 0)
            } < /div> <
            div > Number of parts: {
                this.state.poLines.reduce(
                    (a, b) => a + b.qty17, 0)
