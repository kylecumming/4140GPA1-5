...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        Axios.get(`http://localhost:3000/api/client/getPOs17/${this.state.poNo}`).then((response) => {
            this.setState({ po: response.data });
        }).catch((err) => {
            alert(err);
        });

        Axios.get(`http://localhost:3000/api/client/getPOLines17/${this.state.poNo}`).then((response) => {
