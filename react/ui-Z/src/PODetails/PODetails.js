...
export default class PODetails extends Component {
    ...
    getPOInfomation17() {
        Axios.get(`http://localhost:3000/api/client/z/getPOs17/${this.state.poNo}`).then((response) => {
            this.setState({ po: response.data });
        }).catch((err) => {
            alert(err);
        });

        Axios.get(`http://localhost:3000/api/client/z/getPOLines17/${this.state.poNo}`).then((response) => {
